...
export default function VideoPlayer({
  streamUrl, subtitles = [], poster, title, referer,
}: VideoPlayerProps) {
      ...
      try {
        ...
        if (Hls.isSupported()) {
          ...
          hls.on(Hls.Events.ERROR, (_: any, data: any) => {
            if (cancelled) return;
            console.log("[VideoPlayer] HLS:", data.type, data.details, "fatal:", data.fatal, "url:", (data.url || "").slice(0, 80));
          ...
          video.addEventListener("loadedmetadata", () => { if (!cancelled) setLoading(false); }, { once: true });
          video.addEventListener("error", () => { if (!cancelled) { setError("Failed to load."); setLoading(false); } }, { once: true });
        } else {
        ...
        if (hlsRef.current) {
          ...
          hls.on(Hls.Events.MANIFEST_PARSED, () => {
            ...
            player.config.quality = {
              default: 0, options: [0, ...levels], forced: true,
              onChange(q: number) {
                hls.currentLevel = q === 0 ? -1 : hls.levels.findIndex((l: any) => l.height === q);
              },
            };
            player.quality = 0;
          });
  ...
  return (
    <div className="relative w-full rounded-2xl overflow-hidden ring-1 ring-white/10 shadow-2xl shadow-black/60 bg-black">
      ...
      <div className="aspect-video w-full" style={{
        "--plyr-color-main": "#dc2626", "--plyr-video-background": "#000",
        "--plyr-menu-background": "#18181b", "--plyr-menu-color": "#fff",
        "--plyr-menu-border-color": "#27272a", "--plyr-control-icon-size": "18px",
        "--plyr-font-size-base": "14px", "--plyr-tooltip-background": "#18181b",
        "--plyr-tooltip-color": "#fff", "--plyr-badge-background": "#dc2626",
      } as React.CSSProperties}>
        <video ref={videoRef} className="w-full h-full" crossOrigin="anonymous" playsInline>
          {subtitles.map((sub) => (
            <track key={sub.lang} kind="subtitles" src={sub.url}
              srcLang={sub.lang} label={sub.label}
